...
cluster1color = [51 255 179]/255;
cluster2color = [255 179 51]/255;
lambda = [];
nbins = [];
plottype = 'scatter';
contourFlag = false;
...
if nargin > 2
    ...
    okargs = {'smoothing','bins','plottype','logy','marker','msize','filled','color'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
%             colormap jet;
        ...
        case 5 %'scatter'
            ...
            map = linspace(0,max(col),100)';   %Ran 
            exist color
            if ans == 1
                if color == 1
                    map = [map*0+1,1-map,1-map];    %Ran
                elseif color == 2
                    map = [1-map,map*-0.1+1,1-map];    %Ran
                elseif color == 3 
                    map = [1-map,1-map,map*0+1];    %Ran
                elseif color == 4 
                    map = [map*0+1,1-map,map*0+1];    %Ran
                elseif color == 5 
                    map = [map*0+1,map*-0.2+1,1-map];    %Ran
                elseif color == 6 
                    map = [1-map,map*-0.1+1,map*0+1];    %Ran
                elseif color == 7
                    map = [map*-cluster1color(:,1)+1,map*-cluster1color(:,2)+1,map*-cluster1color(:,3)+1];    %Ran
                elseif color == 8
                    map = [map*-cluster2color(:,1)+1,map*-cluster2color(:,2)+1,map*-cluster2color(:,3)+1];    %Ran
                end
                colormap(map);              %Ran
            end
    end
