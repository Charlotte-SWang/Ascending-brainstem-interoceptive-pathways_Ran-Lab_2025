...
summaryDataDir = ['E:\tempAnalysis\20230626 VLM\summaryFiles'];
...
count2 = 0;
narrowTuneThd = 0;
cellNumberThd = 2;                     % if the number of total responders is smaller than cellNumberThd, skip the FOV
countSkipFile = 0;
countFile = 0;
...
for iFile = 1:fileNumber
    ...
    skipFile(iFile) = 0;
    clear block;
%     if length(stim{1}) == 30              % skip files with only 3 stim to the stomach or files with 30 frames per stim. THESE LINES SHOULD NOT BE INCLUDED WHEN ANALYZING INT CHEM
%         countSkipFile = countSkipFile + 1;
%         continue;skipFile(iFile) = 1;
%     end

%     if length(stimBlocks{1}) ~= 4              % skip files with only 3 stim to the stomach or files with 30 frames per stim
%         countSkipFile = countSkipFile + 1;
%         continue;skipFile(iFile) = 1;
%     end
%             

    %% use the following lines if each stimulus is specified.
% %     register stimuli - edit these lines to analyze stimuli to be included
% 
%     if isempty(find(contains(stimName,'sto'),1,'Last'));
%         skipFile(iFile) = 1;
%     else
%         regStim(1) = find(contains(stimName,'sto'),1,'Last');
%     end
%     if isempty(find(contains(stimName,'int')));
%         skipFile(iFile) = 1;
%     else
%         regStim(2) = find(contains(stimName,'int'),1,'First');
%     end
%         if isempty(find(contains(stimName,'h2o')));
%         skipFile(iFile) = 1;
%     else
%         regStim(1) = find(contains(stimName,'h2o'));
%     end
%     if isempty(find(contains(stimName,'nac')));
%         skipFile(iFile) = 1;
%     else
%         regStim(3) = find(contains(stimName,'nac'));
%     end
%     if isempty(find(contains(stimName,'sal'),1,'Last'));
%         skipFile(iFile) = 1;
%     else
%         regStim(4) = find(contains(stimName,'sal'),1,'Last');
%     end

    
% % %     regStim(2) = find(contains(stimName,'hcl'));
% %     regStim(1) = find(contains(stimName,'sal'),1,'First');
% %     regStim(1) = find(contains(stimName,'hcl'));
% %     regStim(3) = find(contains(stimName,'sal'),1,'Last');
%     regStim(2) = find(contains(stimName,'int'),1,'Last');
% %     regStim(3) = find(contains(stimName,'ili'),1,'Last');
    
%     regStim(1) = find(contains(stimName,'sto'),1,'First');
%     regStim(2) = find(contains(stimName,'sto'),1,'First')+1;
%     regStim(3) = find(contains(stimName,'sto'),1,'First')+2;
%     regStim(4) = find(contains(stimName,'sto'),1,'First')+3;
%     regStim(5) = find(contains(stimName,'int'),1,'First');
%     regStim(6) = find(contains(stimName,'int'),1,'First')+1;
%     regStim(7) = find(contains(stimName,'int'),1,'First')+2;
%     regStim(8) = find(contains(stimName,'sto'),1,'First')+4;
%     regStim(9) = find(contains(stimName,'sto'),1,'First')+5;
%     regStim(10) = find(contains(stimName,'sto'),1,'First')+6;
%     regStim(11) = find(contains(stimName,'sto'),1,'First')+7;
%     regStim(12) = find(contains(stimName,'int'),1,'First')+3;
%     regStim(13) = find(contains(stimName,'int'),1,'First')+4;
%     regStim(14) = find(contains(stimName,'int'),1,'First')+5;
%     regStim = 1:14;
    
%     stimLiBst = {'150','300','600','900','int100','int150','int200','lar'};
%     nStim = length(stimList);
%     for iStim = 1:length(stimList)
%         a = find(contains(stimName,stimList{iStim}),1,'First');
%         if isempty(a) == 1
%             skipFile(iFile) = 1;
%             break;
%         else
...
%     end
    ...
    for iBlock = 1:nBlock
        ...
        if isempty(oriBlockID) == 1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % use the following lines to plot only selected stimuli from the block
            if redefineBlocks == 1
                 if iBlock == 1 % only plotting selected stimuli from the block
                     stimBlocks{oriBlockID} = stimBlocks{oriBlockID}(2);
                 end
%              elseif iBlock ~= 1 % use this and the following lines to plot only one trial per stimulus
%                  stimBlocks{oriBlockID} = stimBlocks{oriBlockID}(end);
%                 end
             % if iBlock == 2                     % int chem
             %     stimBlocks{oriBlockID} = stimBlocks{oriBlockID}(1);
             % end
%              else
%                  stimBlocks{oriBlockID} = stimBlocks{oriBlockID}(3);

%              stimBlocks{oriBlockID} = stimBlocks{oriBlockID}(end);
                goodCellBlock(:,oriBlockID) = any(goodCellEach(:,stimBlocks{oriBlockID}),2)-isBadCell;
                goodCellBlock(goodCellBlock(:,oriBlockID)<0,oriBlockID)=0;
                maxEachBlock(:,block(iBlock)) = max(maxdFFExt(:,stimBlocks{oriBlockID}),[],2);
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
       
        for iStim = 1:length(stimBlocks{oriBlockID})
%         for iStim = 1:min(length(stimBlocks{oriBlockID}),6-length(stimBlocks{oriBlockID}))
%         for iStim = 1:2
%                 timeWindow = [timeWindow stim{stimBlocks{oriBlockID}(iStim)}]; % not used
            count = count + 1;
            regStim(count) = stimBlocks{oriBlockID}(iStim);
                indivWindow = [indivWindow stim{stimBlocks{oriBlockID}(iStim)}(1)-preStimFrame:stim{stimBlocks{oriBlockID}(iStim)}(end)];  % not used
        end        
    end
    if skipFile(iFile) == 1
        countSkipFile = countSkipFile + 1;
...
%         if isBadCell(iCell) == 0
            tempA = min(maxEachBlock(iCell,block(1)),maxEachBlock(iCell,block(2)));
            tempB = max(maxEachBlock(iCell,block(1)),maxEachBlock(iCell,block(2)));
            if tempA ~= 0
                aasd = iCell;
            end
            if tempA/tempB > 0.5 & sum(goodCellBlock(iCell, block),2) == 1
                continue;
                tempC = tempC + 1;
            end
...
%             alldFF(cellCount,:) = dFF(:,iCell);
            ...
            relaRespAll(cellCount,:) = relaResp(iCell,:);
            if narrowTuneThd > 0 & narrowTuneThd < 1            % if so, only consider narrowly-tuned cells
                for iBlock = 1:nBlock
                    if relaRespAll(cellCount,iBlock) < narrowTuneThd
                        goodBlockAll(cellCount,iBlock) = 0;
                        count = count+1;
                    end
                end
            end
...
cor = corrcoef(maxdFFAll);
imagesc(cor,[0 1]);
% colormap parula;
colormap copper;

% load('summary.mat','regClusterList','goodCellAll');
% resList{1} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 1);
% resList{2} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 0);
% resList{3} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 1);
% resList{1} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 0 & goodBlockAll(:,3) == 0);
% resList{2} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 1 & goodBlockAll(:,3) == 0);
% resList{3} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 0 & goodBlockAll(:,3) == 1);
% resList{4} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 1 & goodBlockAll(:,3) == 0);
% resList{5} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 0 & goodBlockAll(:,3) == 1);
% resList{6} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 1 & goodBlockAll(:,3) == 1);
% resList{7} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 1 & goodBlockAll(:,3) == 1);
% resList{1} = find(goodCellAll(:,1) == 1 & goodCellAll(:,2) == 1 & goodCellAll(:,3) == 1);
% resList{2} = find(goodCellAll(:,1) == 1 & goodCellAll(:,2) == 1 & goodCellAll(:,3) == 0);
% resList{3} = find(goodCellAll(:,1) == 1 & goodCellAll(:,2) == 0 & goodCellAll(:,3) == 0);
% resList{4} = find(goodCellAll(:,1) == 0 & goodCellAll(:,2) == 1 & goodCellAll(:,3) == 0);
% resList{5} = find(goodCellAll(:,1) == 1 & goodCellAll(:,2) == 0 & goodCellAll(:,3) == 1);
% resList{6} = find(goodCellAll(:,1) == 0 & goodCellAll(:,2) == 1 & goodCellAll(:,3) == 1);
% resList{7} = find(goodCellAll(:,1) == 0 & goodCellAll(:,2) == 0 & goodCellAll(:,3) == 1);
% resList{1} = 1:cellCount;
% 
resList{1} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 1);
resList{2} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 0);
resList{3} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 1);
% resList{4} = find(goodBlockAll(:,1) == 0 & goodBlockAll(:,2) == 0 & goodBlockAll(:,3) == 1);
% % 
% % 
% resList{5} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,2) == 1 );
% resList{6} = find(goodBlockAll(:,1) == 1 & goodBlockAll(:,3) == 1);
% resList{7} = find(goodBlockAll(:,2) == 1);
% resList{8} = find(goodBlockAll(:,3) == 1);
...
hxx1 = imagesc(allRes(plotList,:),[10 80]); colormap jet;
